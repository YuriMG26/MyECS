...
#include "extras/IconsFontAwesome6.h"  // from https://github.com/juliettef/IconFontCppHeaders
...
inline bool CheckBoxTick(const char* name_, bool* pB_) {
  return CheckBoxFont(name_, pB_, ICON_FA_CHECK, ICON_FA_SQUARE);
}
...
struct FrameTimeHistogram {
  ...
  void PlotRefreshLines(float total_ = 0.0f, float* pValues_ = NULL) {
    ...
    while (xCurr < max.x) {
      float xP =
          ceil(xCurr);  // use ceil to get integer coords or else lines look odd
    ...
    if (pValues_) {
      ...
      for (int i = 0; i < NUM && mark < NUM_MARKERS; ++i) {
        ...
        if (total_ * markers[mark] < currTotal) {
          float xP = ceil(
              (float)(i + 1) / (float)NUM * (max.x - min.x) +
  ...
  void Draw(const char* name_, bool* pOpen_ = NULL) {
    if (ImGui::Begin(name_, pOpen_)) {
      ...
      if (ImGui::CollapsingHeader("Hitch Time Histogram")) {
        ++numShown;
        ImGui::PlotHistogram("", hitchTimes, NUM, 0, NULL, FLT_MAX, FLT_MAX,
                             size);
        PlotRefreshLines();
      }
      if (ImGui::CollapsingHeader("Hitch Count Histogram")) {
        ++numShown;
        ImGui::PlotHistogram("", hitchCounts, NUM, 0, NULL, FLT_MAX, FLT_MAX,
                             size);
        PlotRefreshLines();
      }
